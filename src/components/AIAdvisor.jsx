 ...
 */
export default function AIAdvisor({ theme, mode, accent, graph, compact = false }) {
  ...
  return (
    <div
      style={{
        ...
        padding: 14, flex: compact ? undefined : 1,
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
          ...
          <p style={{ fontSize: 13, fontWeight: 500, color: txt, fontFamily: fontBody }}>AI Advisor</p>
        </div>
        <motion.button
          whileTap={{ scale: 0.95 }}
          onClick={getAIAdvice}
          disabled={loading}
          style={{
            fontSize: 11, fontFamily: fontBody, padding: "4px 10px", borderRadius: 6,
      ...
      {!compact && (
        <p style={{ color: sub, fontSize: 11, marginBottom: 12, fontFamily: fontBody }}>
      ...
      {/* Body */}
      {advice || loading ? (
        <div style={{ background: inputBg, borderRadius: 6, padding: 10, flex: 1 }}>
          <div
            style={{
              fontSize: 9, fontWeight: 600, textTransform: "uppercase",
              letterSpacing: ".7px", color: sub, marginBottom: 8, fontFamily: fontBody,
            }}
          >
            {mode} analysis
          </div>
          {loading && !advice
            ? [1, 2, 3].map((i) => (
                <div
                  ...
                  style={{
                    height: 10, borderRadius: 6, background: theme.bdr,
                    width: i === 3 ? "60%" : "100%", opacity: 0.5, marginBottom: 8,
                  }}
                />
              ))
            : (
                  ...
                  <div key={i} style={{ display: "flex", gap: 7, alignItems: "flex-start" }}>
                    <span
                      style={{
                        ...
                        background: dark ? "rgba(59,109,17,0.15)" : "#EAF3DE", color: "#27500A",
                      }}
                    >
                      {i + 1}
                    </span>
                    <p style={{ color: sub, fontSize: 11, lineHeight: 1.55, margin: 0, fontFamily: fontBody }}>
                      {line}
                    </p>
                  </div>
                ))}
        ...
        <div
          style={{
            background: inputBg, borderRadius: 6, padding: "14px 12px",
