...
/**
 * Shared layout for Traffic, Energy, Water, Disaster, and Analytics pages.
 *
 * Structure:
 *   4-stat row → alert banner → [map card | right panel (metrics, AI, alerts, history)]
 */
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10, flexShrink: 0 }}>
        {stats.map((stat) => (
          ...
          >
            ...
            <p style={{ fontSize: 19, fontWeight: 500, fontFamily: fontMono, color: txt }}>{stat.value}</p>
            <Badge label={stat.badge} type={stat.badgeType} theme={theme} />
          </div>
        ))}
      </div>

      {/* Alert banner (only shown when relevant) */}
      <AlertBanner
        theme={theme}
        mode={mode}
          ...
          >
            <div>
              <p style={{ fontSize: 13, fontWeight: 500, color: txt }}>
                City Road Network
                <span style={{ color: sub, fontWeight: 400, fontSize: 12, marginLeft: 8 }}>
                  {currentMode.label} Mode
                </span>
              </p>
              <p style={{ color: sub, fontSize: 11, marginTop: 2 }}>
                {start
                  ? `Origin: ${originLabel} — select destination`
          ...
          <AnimatePresence>
            {pathError && (
              <motion.div
                ...
                exit={{ opacity: 0 }}
                style={{
                  borderTop: "1px solid rgba(226,75,74,0.25)",
                  background: dark ? "rgba(163,45,45,0.08)" : "#FCEBEB",
                  color: "#A32D2D", padding: "7px 14px", fontSize: 12, flexShrink: 0,
              ...
              >
                ...
                <button
                  ...
                  style={{ marginLeft: "auto", opacity: 0.6, background: "none", border: "none", cursor: "pointer" }}
                >
                  <X size={11} />
                </button>
              </motion.div>
          ...
          <AnimatePresence>
            {path.length > 0 && (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                style={{
                  borderTop: `0.5px solid ${bdr}`, padding: "8px 14px", fontSize: 12, flexShrink: 0,
                  display: "flex", alignItems: "center", gap: 6,
                }}
              >
                ...
                <span style={{ color: "#3B6D11", fontWeight: 600 }}>
                  {path
                    .map((id) => graph.nodes.find((n) => n.id === id)?.label?.split(" ")[0])
                    .join(" → ")}
                </span>
